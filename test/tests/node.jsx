...
function objectRenderer(renderNode : ComponentNode<*> | ElementNode | TextNode) : Object {
...
describe('node render cases', () => {
    ...
    it('should be able to render an element node', () => {
        ...
        if (children[0].text !== 'baz') {
            throw new Error(`Expected child text to be 'baz', got '${ children[0].text }'`);
        }
    });
        ...
        children.forEach(innerNode => {
            if (innerNode.name === 'ul') {
                if (innerNode.children.length !== 2) {
                    throw new Error(`Expected ul to have 2 children, got ${ innerNode.children.length }`);
                }
            }
        });
    });
    ...
    it('should be able to render a function returning a list of element nodes', () => {
        ...
        if (children[0].name !== 'button') {
            throw new Error(`Expected first child to be button, got ${ children[0].name }`);
        }
        ...
        if (children[2].name !== 'section') {
            throw new Error(`Expected third child to be section, got ${ children[0].name }`);
        }
    });
